...
class AGRAWALGenerator(Stream):
    ...
    def __init__(self, classification_function=0, random_state=None, balance_classes=False,
                 perturbation=0.0, imbalance_ratio=-1):
        super().__init__()
        ...
        self._classification_functions = [self._classification_function_zero,
                                          ...
                                          self._classification_function_ten,
                                          self._classification_function_eleven,
                                          ]
    ...
    def classification_function(self, classification_function_idx):
        ...
        classification_function_idx: int
            from 0 to 10
        """
        print(f"classification_function_idx: {classification_function_idx}")
        if classification_function_idx in range(11):
            ...
            while not desired_class_found:
                ...
                if not self.balance_classes:
                    if self.imbalance_ratio > 0:
                        if rand_val < self.imbalance_ratio:
                            if group == 0:
    ...
    @staticmethod
    def _classification_function_ten(salary, commission, age, elevel, car, zipcode, hvalue,
                                      hyears, loan):
        """ classification_function_zero
        ...
        """
        # TODO
        return 0 if ((age < 40) or (60 <= age) or car <= 2) else 1

    @staticmethod
    def _classification_function_eleven(salary, commission, age, elevel, car, zipcode, hvalue,
                                       hyears, loan):
        """ classification_function_seven
        ...
        # TODO
        disposable = (2 * (salary + commission) / 3 - 5000 * elevel - 20000)
        return 0 if disposable > 1 or car <= 2 else 1
