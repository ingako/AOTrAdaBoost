...
for seed in range(10):
    disable_output = f"{base_dir}/disable_transfer/{seed}/result-stream-1.csv"
    disable_df = pd.read_csv(disable_output)
    disable_acc_list.append(disable_df["accuracy"])
...
result_line.append("seed_count,acc-gain-std,acc-gain-mean\n")
    ...
    if len(param_values) != len(param_strs):
        ...
        for seed in range(10):
            ...
            for i in range(0, end):
                tradaboost_disable_gain += tradaboost_acc[i] - disable_acc_list[seed][i]

                if i == (end - 1):
                    result_list.append(tradaboost_disable_gain)
        mean = np.mean(result_list)
        std = np.std(result_list)
        result_line = [f"{param_values[i]}" for i in range(len(param_strs))]
        result_line.append(f"{len(result_list)},{std},{mean}\n")
        gain_report_out.write(",".join(result_line))
